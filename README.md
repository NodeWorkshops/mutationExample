...
Create a folder and you'll need two JSON files and two Javascript files inside. The instructions will prompt you when to create these. 
...
2. Run: npm install express express-graphql graphql --save [Enter]
        npm install json-server axios --save [Enter]
        npm install nodemon [Enter]
...
6. Create a new file called: schema.js and build out your schema. Making sure you define all the types you'd like to use. Then building out the schema that you will use to query in GraphiQL including your resolvers. It should look like this...
...
8. Create a new terminal keeping your old one running. In this new terminal window run the second script we created in the Package.json file. Input: 
...
13. Try and add a delete References Type in that is similar to the last but removing the args. 
 ...
 15. Our final mutation is to edit the data. Create another type and resolver similar to the last. Ensuring only the ID is non null so you just need this information to change any field.
